	...
	public partial class Reportes : Form
	{
		...
		private void crearDocumento()
		{
			...
			//try
			{
                ...
                if (I.Contains(true))
                {
                    procesoI(doc, formatoTituloColumna);
                }
        ...
        private void procesoI(DocX doc, Formatting formatoTituloColumna)
        {
            bool band = true;
            ...
            // -------------------------------------------------------------------------------
            co.Comando("SELECT COUNT(*) FROM Pagos WHERE ingresoID IS NOT NULL AND FechaPago BETWEEN '" + fechaA + "' AND '" + fechaB + "';");
            if (co.LeerRead)
                renglones = co.Leer.GetInt32(0);

            if (renglones != 0)
            {
                doc.InsertParagraph("Ingresos", false, formatoTituloColumna);
            }
            else
            {
                doc.InsertParagraph("No hay ingresos", false, formatoTituloColumna);
                return;
            }
            ...
            if (indices.Count == 1 && indices[0] == 5)
            {
                ...
                t.Rows[t.RowCount - 1].Cells[columnas - 1].Paragraphs.First().Append("$ " + totalI());
                band = false;
            }
            ...
            if (band)
            {
                ...
                for (int col = 0; col < columnas; col++)
                    if (indices[col] != 5)
                        t.Rows[0].Cells[col].Paragraphs.First().Append(columnasI(indices[col]));

                //De los pagos realizados se obtien el ID, nombre del empleado y trabajo, fecha de pago
                co.Comando("SELECT p.ID, p.Monto, i.Concepto, m.Metodo, concat(DAY(p.FechaPago), '/', MONTH(p.FechaPago), '/', YEAR(p.FechaPago)) " +
                                       "FROM Pagos AS p INNER JOIN Metodo AS m ON(m.ID = p.MetodoID) " +
                                       "INNER JOIN Ingresos AS i ON(i.ID = p.ingresoID) " +
                                       "WHERE p.ingresoID IS NOT NULL " +
                ...
                while (co.LeerRead)
                {
                    ...
                    {
                        if (indices[col] != 5)
                            t.Rows[i + 1].Cells[col].Paragraphs.First().Append(valoresI(indices[col]));
                    }
                    i++;
                }
                if (indices.Contains(5))
		...
		private int arreglos()
		{
            ...
            I[0] = checkBox12.Checked;
            I[1] = checkBox28.Checked;
            I[2] = checkBox23.Checked;
            I[3] = checkBox22.Checked;
            I[4] = checkBox21.Checked;
            I[5] = checkBox20.Checked;
			...
			foreach (bool check in GR)
				if (check == false)
					c++;
            foreach (bool check in I)
                if (check == false)
                    c++;

            if (c == (PPP.Count + PPC.Count + GR.Count + PR.Count + I.Count))
				return 0;
			else
				return 1;
		}
        ...
        private String columnasI(int col)
        {
            String nombre = "";
        ...
        private String valoresI(int col)
        {
            String valor = "";
        ...
        {
            int total = 0;
            co.Comando("SELECT totalI('" + fechaA + "', '" + fechaB + "');");
            if (co.LeerRead)
                total = co.Leer.GetInt32(0);
            return total + "";
        }
