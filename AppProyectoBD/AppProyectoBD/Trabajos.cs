...
namespace AppProyectoBD
{
    ...
    public partial class Trabajos : Form
    {
        ...
        //Modificar días en fechas al hacer un update en trabajos  **checar**
        public void DatosTablas()
        {
            //------------------Mostrar datos en tablas Tabajos ----Trabajos actuales----------------------
            //Cuento los números de Trabajos en dicha fecha
            int rows = 0;
            ...
            co.Comando("SELECT t.ID, t.nombre, tt.NombreTipoTrab ,IFNULL(p.Nombre,'Sin proyecto'), t.FechaFin FROM Proyectos as p " +
        ...
        private void buscarTrabajos(string fechaA, string fechaB, DataGridView dataGridView, int sel)
        {
            ...
            co.Comando("SELECT COUNT(*) FROM Trabajos WHERE FechaFin "+simbolo+" CURDATE() AND FechaFin  between '" + fechaA + "' AND '" + fechaB + "';");
            int rows = 0;
            if (co.LeerRead)
                rows = co.Leer.GetInt32(0);

            //Coloco los renglones en los datagridview según sea el caso
            if (rows > 0)
            ...
            int i = 0;
            co.Comando("SELECT t.ID, t.nombre, tipo.NombreTipoTrab, IFNULL(p.Nombre,'Sin proyecto'), t.FechaFin FROM Trabajos AS t " +
                       "LEFT OUTER JOIN Proyectos AS p ON ( t.ProyectosID = p.ID) " +
                       "INNER JOIN TipoTrabajos AS tipo ON (t.TipoTrabajosID = tipo.ID) " +
                       "WHERE t.FechaFin " + simbolo + " CURDATE() AND t.FechaFin between '" + fechaA + "' AND '" + fechaB + "';");
